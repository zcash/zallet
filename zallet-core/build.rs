...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    #[cfg(not(zallet_build = "merchant_terminal"))]
    generate_rpc_book_reference(&out_dir)?;
...
/// Generates the JSON-RPC method reference for the Zallet book, as Markdown.
///
/// The output is pinned to `book/src/rpc/methods.md` by the `book_rpc_reference`
/// integration test, so that the book always matches the rustdoc on the RPC traits.
#[cfg(not(zallet_build = "merchant_terminal"))]
fn generate_rpc_book_reference(out_dir: &Path) -> Result<(), Box<dyn Error>> {
    // Parse the source file containing the RPC traits.
    ...
    // (method name, doc lines, deprecated, wallet-builds only)
    let mut methods: Vec<(String, Vec<String>, bool, bool)> = vec![];

    for (rpc_trait, wallet_only) in [(find_trait("Rpc"), false), (find_trait("WalletRpc"), true)] {
        for item in &rpc_trait.items {
            if let syn::TraitItem::Fn(method) = item {
                ...
                if let Some(command) = command {
                    ...
                    methods.push((command, doc_lines, deprecated, wallet_only));
                }
            }
        }
    }
    ...
    for (command, doc_lines, deprecated, wallet_only) in methods {
        contents.push_str("\n## `");
        contents.push_str(&command);
        contents.push_str("`\n\n");

        if wallet_only {
            contents.push_str("*Only available in wallet builds of Zallet.*\n\n");
        }
        if deprecated {
            contents.push_str("> This method is deprecated.\n\n");
        }

        for line in doc_lines {
            // Demote rustdoc section headings below the per-method headings.
    ...
    fs::write(out_dir.join("rpc_methods.md"), contents)?;
