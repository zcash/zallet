...
mod range_loading_tests {
    ...
    impl ChainView for RangeView {
        ...
        fn stream_blocks(
            ...
            let first_block = Ok(block(range.start));
            if self.expires_during_stream {
                ...
                stream::iter(vec![first_block, Ok(block(range.start + 1))]).boxed()
            }
        }
    ...
    fn block(height: BlockHeight) -> Block {
        let block_tag =
            u8::try_from(u32::from(height)).expect("test block height fits in one byte");
        let header = BlockHeaderData {
            version: 4,
            prev_block: BlockHash([block_tag.saturating_sub(1); 32]),
            merkle_root: [0; 32],
