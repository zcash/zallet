...
pub(super) const PARAM_MESSAGE_DESC: &str = "The message to create a signature of.";
...
pub(crate) async fn call(
    ...
    let transparent_addr = TransparentAddress::decode(wallet.params(), t_addr)
        .map_err(|_| LegacyCode::InvalidAddressOrKey.with_static("Invalid Zcash address"))?;
    ...
    match metadata.source() {
        ...
        TransparentAddressSource::Derived {
            ...
            if scope == TransparentKeyScope::EPHEMERAL {
                return Err(LegacyCode::Wallet.with_static("Private key not available"));
            }
...
fn map_wallet_locked_error(e: crate::error::Error) -> jsonrpsee::types::ErrorObject<'static> {
    match e.kind() {
        ...
        crate::error::ErrorKind::Generic if e.to_string() == "Wallet is locked" => {
            LegacyCode::WalletUnlockNeeded.with_static(
